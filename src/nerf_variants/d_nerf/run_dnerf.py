...
sys.path.append(r"C:\Users\mdopiriak\PycharmProjects\iis_nerf\src\d_nerf\D_NeRF")
...
def render_all(H, W, focal, chunk=1024*32, rays=None, c2w=None, ndc=True,
                  near=0., far=1., frame_time=None,
                  use_viewdirs=False, c2w_staticcam=None,
                  **kwargs):
    """Render rays
...
def render_path_all(render_poses, render_times, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None,
    ...
    for i, (c2w, frame_time) in enumerate(zip(tqdm(render_poses), render_times)):
        rgb, disp, acc, depth, _ = render_all(H, W, focal, chunk=chunk, c2w=c2w[:3,:4], frame_time=frame_time, **render_kwargs)
...
def config_parser():
    ...
    parser.add_argument("--chunk", type=int, default=1024*40,
                        help='number of rays processed in parallel, decrease if running out of memory')
    parser.add_argument("--netchunk", type=int, default=1024*70,
